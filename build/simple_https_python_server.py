...
def get_cert_and_key():
    ...
    certificate = builder.sign(private_key=private_key, algorithm=SHA256(), backend=default_backend())

    temp_dir = tempfile.mkdtemp()
    print(f"Generating temporary certificates in {temp_dir}")
...
parser.add_argument("-u", "--unsecure", action="store_true", help="Use HTTP instead of HTTPS (default: HTTPS)")
args = parser.parse_args()

# Set port and server configuration
local_ip = get_local_ip()
if args.unsecure:
    server_address = ("0.0.0.0", 8080)
    httpd = http.server.HTTPServer(server_address, MyRequestHandler)
    print(f"Serving on http://{local_ip}:{server_address[1]}/")
else:
    ...
    print(f"Serving on https://{local_ip}:{server_address[1]}/")
